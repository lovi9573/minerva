...
def train_network(model, num_epochs=100, minibatch_size=256, lr=0.01, mom=0.75, wd=5e-4):
    # load data
    (train_data, test_data) = mnist_io.load_mb_from_mat('mnist_all.mat', minibatch_size / len(node))
    ...
    for i in xrange(num_epochs):
        ...
        last = time.time()
        count = 0
        weightgrads = [None] * len(node)
        biasgrads = [None] * len(node)
        for (mb_samples, mb_labels) in train_data:
            count += 1
            current_node = count % len(node)
            owl.set_device(node[current_node])
            num_samples = mb_samples.shape[0]
            data = owl.from_numpy(mb_samples).reshape([28, 28, 1, num_samples])
            label = owl.from_numpy(mb_labels)
            out, weightgrads[current_node], biasgrads[current_node] = bpprop(model, data, label)
            if current_node == 0:
                for k in range(len(model.weights)):
                    model.weightdelta[k] = mom * model.weightdelta[k] - lr / num_samples / len(node) * multi_node_merge(weightgrads, 0, k) - lr * wd * model.weights[k]
                    model.biasdelta[k] = mom * model.biasdelta[k] - lr / num_samples / len(node) * multi_node_merge(biasgrads, 0, k)
                    model.weights[k] += model.weightdelta[k]
                    model.bias[k] += model.biasdelta[k]
                if count % (len(node) * lazy_cycle) == 0:
...
def multi_node_merge(l, base, layer):
    if len(l) == 1:
        return l[0][layer]
    left = multi_node_merge(l[:len(l) / 2], base, layer)
    right = multi_node_merge(l[len(l) / 2:], base + len(l) / 2, layer)
    owl.set_device(base)
    return left + right
...
if __name__ == '__main__':
    ...
    print 'Using %d CPU(s)' % nodes
    node = [owl.create_mpi_device(i,0) for i in range(nodes)]
    #owl.set_device(node[0])
