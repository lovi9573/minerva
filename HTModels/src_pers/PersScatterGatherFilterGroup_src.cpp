...
#include "Ht.h"
#include "PersScattergatherfiltergroup.h"
...
void
CPersScattergatherfiltergroup::PersScattergatherfiltergroup()
{
	if (PR_htValid) {
		switch (PR_htInst) {
		...
		case SG_FILTER_GROUP_MERGE: {
			...
			uint64_t mask = 0x000000000000FFFFl << shift;
		...
		case SG_FILTER_GROUP_WRITE: {
			...
			//printf("write activation: should be 5066626891382802 %lu; => %lu \n",PR_activation_group,PR_out_Addr);
			WriteMem(PR_out_Addr, P_activation_group);
			WriteMemPause(SG_FILTER_GROUP_RTN);
		}
		break;
		case SG_FILTER_GROUP_RTN: {
			BUSY_RETRY(SendReturnBusy_scattergatherfiltergroup());
			SendReturn_scattergatherfiltergroup();
		}
		break;
		default:
			assert(0);
